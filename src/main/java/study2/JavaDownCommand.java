...
public class JavaDownCommand implements StudyInterface {
	...
	public void execute(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		String fName = request.getParameter("file"); // 자바로 파일 이름명 가져오는것
		
		File file = new File(realPath + fName);  //realPath에 있는 + 파일명
		
		// 프로토콜형식에 맞도록 헤더에 정보를 제공 해준다.
		...
		// 사용하는 브라우저에 대한 정보 : 만약에 인터넷익스플로서(IE)인 경우는 'euc-kr', 'utf-8'로 전송한다.
		String downLoadName = "";
		...
		// Java에 의해서 실제로 파일을 다운(업)로 처리시켜준다.
		FileInputStream fis = new FileInputStream(file);
		// FileOutputStream fos = new FileOutputStream(nFile); 서버에서 서버로 , FileOutputStream : 서버로 가는것
