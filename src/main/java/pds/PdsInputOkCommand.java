...
public class PdsInputOkCommand implements PdsInterface {
	...
	public void execute(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		//hasmoreelement 는 안에 들어있는 객체만큼 있으면 반복
		while(fileNames.hasMoreElements()) {
			String file = (String) fileNames.nextElement();
			
			if(multipartRequest.getFilesystemName(file) != null) {
				originalFileName += multipartRequest.getOriginalFileName(file) + "/";  // 실제 파일명들을 누적해서 저장
				filesystemName += multipartRequest.getFilesystemName(file) + "/";			// 서버에 올라가는 파일명들을 누적해서 저장				
			}
		}
