...
%>
	...
	<c:forEach var="num" items="${num1}" varStatus="st">
		${st.count}(num[(${st.index}]). ${num} <br />  <!-- ${st.count} : 수행 횟수를 적음 (1. 2.) /// ${st.index}인덱스 수행횟수 (0부터) -->
	</c:forEach>
	<br />
	<h4>EL에서의 연산처리(연산자 사용)</h4>
	1. num1[0] == num2[0] : ${num1[0] == num2[0]} <br />  <!-- 맞으면 true // 틀리면 false 나옴! -->
	...
	7. num1[1] gt num2[1] : ${num1[1] gt num2[1]} <br /> <!-- greater than? -->
...
%>	
	...
	== 비교 : ${var1== ""} <br />
	== 비교 : ${var2== ""} <br />
	== 비교 : ${var3== ""} <br /> <!-- null이라는 값?글자가 들어있다고 판단  -->
